...
def count_digits(data_list, method):
    ...
    for sample in data_list:
        ...
        if method == "first":
            digits[sample[0]] += 1
        elif method == "second" and len(sample) > 1:
            digits[sample[1]] += 1
        else:
            continue
    if method == "first":
        digits.pop("0", None)
    data_count = [v for (k, v) in sorted(digits.items())]
...
def main():
    while True:
        ...
        try:
            ...
            break
    data_count, data_pct, total_count = count_digits(data_list, "first")
    ...
    if chi_square_test(data_count, first_expected_counts, "first"):
        print("Observed distribution matches expected distribution for first digit.")
    else:
        print("Observed distribution does not match expected for first digit.", file=sys.stderr)

    second_data_count, second_data_pct, second_total_count = count_digits(data_list, "second")
    ...
    if chi_square_test(second_data_count, second_expected_counts, "second"):
        print("Observed distribution matches expected distribution for second digit.")
    else:
        print("Observed distribution does not match expected for second digit.", file=sys.stderr)
...
# I had to wrangle some data due to the specific way the Louisiana SOS formats their files.
